...
### <a name="specifics">specificsフィールドのキー</a>
違うキー同士の組の順序に意味はない。
同名キー同士の組の順序は有意である場合があるため、同名キー同士の組の順序を変更してはならない。

| キー               | 1レコード中の数 | 値                |                                                        |
|--------------------|-----------------|-------------------|--------------------------------------------------------|
| no-pixelization    | 0〜1個。        | 空文字列。        | 指定されていれば、画像のモザイク処理 (ピクセル化) を行わない。 |
| magnification      | 0〜1個。        | 0より大きい実数。 | 画像の拡大率。                                         |
| last-magnification | 0〜1個。        | 0より大きい実数。 | このキーを指定する場合は、magnificationキーの指定も必須。解答時間が残り0秒になったときの、画像の拡大率。解答時間が減るにつれて拡大率を変動させたい場合に指定。 |
...
| volume             | 0〜1個。        | 0より大きい実数。 | 音声、動画ファイルの音量。基準を1として何倍とするか。  |
| require-all-right  | 0〜1個。        | 空文字列。        | 選択問題で複数選択する必要がある場合に指定。           |
| score              | 0〜1個。        | 0以上の整数。     | 該当レコードの問題の得点。                             |
| last-score         | 0〜1個。        | 0以上の整数。     | このキーを指定する場合は、scoreキーの指定も必須。解答時間が残り0秒になったときの、問題の得点。解答時間が減るにつれて得点を変動させたい場合に指定。 |
...
* 辞書ファイルを再出力する際、specificsフィールドの未知のキーを除去してはならない。
  また、同名キーの出現数が制限されている場合も、制限を超えた部分の値を除去してはならない。
